...
RETURNS TRIGGER AS $$
BEGIN
    -- Automatically hash the password before inserting/updating
    IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.pw_hash IS DISTINCT FROM OLD.pw_hash) THEN
        NEW.pw_hash := crypt(NEW.pw_hash, gen_salt('bf')); -- Using bcrypt for hashing
    END IF;
    RETURN NEW;
END;
$$
...
-- Create a trigger for hashing passwords
CREATE TRIGGER hash_password_trigger
BEFORE INSERT OR UPDATE OF pw_hash ON LUser
FOR EACH ROW
